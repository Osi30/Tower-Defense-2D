...
public class Login : MonoBehaviour
{
    ...
    private string loginEndpoint = "/api/Auth/login"; //endpoint login
    ...
    private void OnLoginClicked()
    {
        ...
        StartCoroutine(LoginAction(u, p));
    }

    public IEnumerator LoginAction(string username, string password)
    {
        var reqData = new LoginRequest { username = username, password = password };
        string jsonData = JsonUtility.ToJson(reqData);

        var uwr = new UnityWebRequest(baseUrl + loginEndpoint, "POST");
